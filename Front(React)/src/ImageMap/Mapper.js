...
const ImageMap = () => {
    ...
    const [cursorStyle, setCursorStyle] = useState('cursor-default'); // 기본 커서 스타일
    ...
    const handleMouseEnter = () => {
        setCursorStyle('cursor-pointer'); // 마우스가 특정 구역에 닿을 때 커서 변경
    };

    const handleMouseLeave = () => {
        setCursorStyle('cursor-default'); // 마우스가 특정 구역을 벗어날 때 커서 원래대로 변경
    };
