...
// 게시글 수정
const BoardEdit = ({ idx, backToList, myboard }) => {
    ...
    const saveBoard = async () => {
        ...
        try {
            await axios.post(`${url}edit?idx=${idx}`, JSON.stringify(board), config);
            alert('수정되었습니다.');
            backToList();
        } catch (error) {
    ...
    return (
        <div className="flex h-full bg-white items-center justify-center overflow-hidden">
            <div className="w-full bg-white rounded px-5">
                <header>
                    {myboard ? <></> : <h2 className="text-4xl font-bold text-center text-slate-700">게시물 수정</h2>}
                </header>
                <form>
                    <div className="mb-4">
